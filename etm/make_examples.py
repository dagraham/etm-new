...
def make_examples(egfile: str = None, num_items: int = num_items, last_id=0):
    ...
    doghouse_example = f"""\
- doghouse @s {now.strftime("%Y-%m-%d")} @e 5d @d #lorem
...
"""
    ...
    #     goal_examples = f"""\
    # ~ {phrase()} @s {now.strftime("%Y-%m-%d")} @q 2w @daylight lorem
...
""".split("\n")
    ...
    for i in range(4):
        examples.append(
            f"- {phrase()} {beg()} {random.choice(freq)} {random.choice(stop)} {cnt()} @d #lorem"
        )
    ...
    for client in clients:
        ...
        examples.append(f"% {summary} @i {i} @d {d} #lorem")
    ...
    while len(examples) < num_items:
        ...
        if t == "%" and start <= now:
            ...
            if summary not in daily:
                ...
                examples.append(
                    f"{t} {summary} @i #daily @d {description} #lorem\n\n  * {note_time()}\n    - {phrase()}"
                )
