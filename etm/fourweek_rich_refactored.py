...
# SELECTED_BACKGROUND = "#505050"
# SELECTED_BACKGROUND = "#585858"
# SELECTED_BACKGROUND = "#606060"
# SELECTED_BACKGROUND = "#686868"
SELECTED_BACKGROUND = NAMED_COLORS["DimGrey"]  # aka "#696969"
...
class FourWeekView:
    ...
    def generate_table(self, grouped_events):
        ...
        now = datetime.now()
        now_year, now_week, now_day = now.isocalendar()
        ...
        while current_date <= end_date:
            ...
            row_style = f"on {SELECTED_BACKGROUND}" if SELECTED else ""
            # row_style = f"reverse" if SELECTED else ""
...
Movement: up/down 1 week, left/right 4 weeks 
1, 2, 3, 4: jump to week number, 0: restore item list
a, b, ...: display details for tagged item[/{DIM_COLOR}]"""
    ...
    def get_week_details(self, yr_wk):
        ...
        for i in range(7):
            ...
            weekday_to_events.setdefault(this_day, [])
        ...
        for day, events in weekday_to_events.items():
            if events:
                flag = (
                    f" [bold][{TODAY_COLOR}](today)[/{TODAY_COLOR}][/bold]"
                    if day == today
                    else f" [bold][{TODAY_COLOR}](tomorrow)[/{TODAY_COLOR}][/bold]"
                    if day == tomorrow
                    else ""
                )
