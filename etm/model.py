...
# from etm.view import set_lexer
...
def create_item_views(item):
    ...
    if item['itemtype'] == '!':
        ...
        if 'r' in item:
            ...
            relevant = dtstart
            if (
                logger.debug(f"{relevant = }; {today = }; {possible_beginby = }") and
                possible_beginby and
                ZERO < relevant.date() - today.date() <= possible_beginby
            ):
                beginbys.append(
                    [
                        (relevant.date() - today.date()).days,
