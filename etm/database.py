...
def use_examples():
    ...
    while len(records) < num_items:
        ...
        dts = (
            ...
            if date
            else start.strftime('%Y%m%dT%H%M00')
        )
    ...
    for record in records:
        id += 1
        print(f"Inserting record using UTC times: {record}") 
        c.execute(
...
def generate_datetimes(rrule_string, extent, start, end):
    ...
    for start in occurrences:
        ...
        event_start = start.replace(tzinfo=gettz('UTC')).astimezone().replace(tzinfo=None) 
    ...
    for record_id, rrulestr_str, extent, processed in records:
        ...
        for occ in occurrences:
            ...
            c.execute(
                ...
                """,
                (record_id, round(start_datetime.timestamp()), round(end_datetime.timestamp()))
            )
...
def get_events_for_period(start_date, end_date):
    ...
    print(f"between {datetime.fromtimestamp(start_ts) = } and {datetime.fromtimestamp(end_ts) = }...")  
...
def process_events(events):
    ...
    for record_id, start_ts, end_ts in events:
        start_dt = datetime.utcfromtimestamp(start_ts).replace(tzinfo=gettz('UTC')).astimezone().replace(tzinfo=None)
        end_dt = datetime.utcfromtimestamp(end_ts).replace(tzinfo=gettz('UTC')).astimezone().replace(tzinfo=None) 

        while start_dt.date() <= end_dt.date():
            # Compute the end time for the current day
            day_end = min(
                end_dt,
                datetime.combine(start_dt.date(), datetime.max.time())
            )
...
def render_weekday_table(grouped_events, start_year, start_week, num_weeks):
    ...
    table.add_column("Week", justify="center", style="dim", no_wrap=True, width=6, vertical="middle")
    for day in weekdays:
        table.add_column(day, justify="center", style=DAY_COLOR, no_wrap=True, width=10, vertical="middle") 
...
def get_busy_bar(lop: List[Tuple[int, int]]) -> Tuple[str, str]:
    ...
    for (b, e) in busy_minutes:
        ...
        for i in range(start, end+1):
            ...
            print(f"{i = }, {b = }, {e = }, {SLOT_MINUTES[i] = }, {busy_conf[SLOT_MINUTES[i]] = }") 
    ...
    for (b, e) in conflict_minutes:
        ...
        for i in range(start, end+1):
            ...
            print(f"{b = }, {e = }, {SLOT_MINUTES[i] = }, {busy_conf[SLOT_MINUTES[i]] = }") 
