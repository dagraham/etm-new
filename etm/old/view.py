...
# to highlight the current month and day in calendar view
...
To reschedule enter the {new} """
    ...
    def coroutine():
        ...
        if new_datetime:
            try:
                ok, dt, z = parse_datetime(new_datetime, z='local')
                logger.debug(f"{new_datetime = } -> {ok = } {dt = } {z = }")
            except:
                ok = False

            if ok and isinstance(dt, datetime):
                logger.debug(f"new datetime: {dt = }")
                changed = item.reschedule(doc_id, instance, dt)
            else:
                changed = False
                show_message('cancelled - invalid', f"'{new_datetime}' is not a valid datetime entry")
